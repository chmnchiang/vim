...
function M.setup(use)
  use({
    ...
    config = function()
      ...
      vim.cmd(
        string.format(
          [[highlight CommentSign guifg=#CCCCCC guibg=%s]],
          utils.get_highlight_color('SignColumn', 'bg')
        )
      )
      vim.fn.sign_define('METEOR_COMMENTS_ICON', { text = ' ', texthl = 'CommentSign' })
      comments.setup({
      ...
      utils.simap('n', ']xc', [[<Cmd>lua require('google.comments').goto_next_comment()<CR>]])
      utils.simap('n', '[xc', [[<Cmd>lua require('google.comments').goto_prev_comment()<CR>]])
    end,
  })
  use({
